...
public class JT808Controller {
    ...
    //added by wgx
    @Operation(summary = "8F04 平台报警应答")
    @PostMapping("alarm_reply")
    public T0001 sendAlarmReply(
                        @Parameter(description = "终端手机号") @RequestParam String clientId,
                        @Parameter(description = "流水号") @RequestParam Integer flowNum,
                        @Parameter(description = "报警类型ID") @RequestParam Integer alarmType,
                        @Parameter(description = "事件项编码") @RequestParam Integer eventType) {
        T8F04 request = new T8F04(clientId);
        request.setFlowNumber(flowNum);
