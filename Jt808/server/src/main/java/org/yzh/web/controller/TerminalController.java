...
public class TerminalController {
    ...
    public T0001 sendText(@ApiParam("终端手机号") @RequestParam String clientId,
        ...
        request.setSign(new int[]{0,0,0,1,0,0,0,0});
        request.setContent(content);
        T0001 response = messageManager.request(request, T0001.class);
        return response;
    }

    @ApiOperation(value = "音视频参数下发", tags = "信息类协议")
    @PostMapping("media")
    public T0001 sendText(@ApiParam("终端手机号") @RequestParam String clientId,
                          @ApiParam("多媒体ID") @RequestParam int mediaId,
                          @ApiParam("多媒体类型") @RequestParam int mediaType,
                          @ApiParam("事件项编码") @RequestParam int eventType,
                          @ApiParam("Token") @RequestParam String token) {
        T8F03 request = new T8F03(clientId);
    ...
    @ApiOperation(value = "拍照指令下发", tags = "信息类协议")
    @PostMapping("shoot")
    public T0001 sendText(@ApiParam("终端手机号") @RequestParam String clientId,
                          @ApiParam("开始时间（yyyy-MM-dd HH:mm:ss）") @RequestParam LocalDateTime dateTime) {
        T8F02 request = new T8F02(clientId);
        request.setDateTime(dateTime);
        T0001 response = messageManager.request(request, T0001.class);
        return response;
    }

    @ApiOperation(value = "接收拍照上报消息应答", tags = "信息类协议")
    @PostMapping("pzyd")
    public T0001 sendText(@ApiParam("终端手机号") @RequestParam String clientId,
                          @ApiParam("应答流水号") @RequestParam int flowId,
                          @ApiParam("多媒体ID") @RequestParam int mediaId,
                          @ApiParam("结果") @RequestParam int result) {
        T8F01 request = new T8F01(clientId);
