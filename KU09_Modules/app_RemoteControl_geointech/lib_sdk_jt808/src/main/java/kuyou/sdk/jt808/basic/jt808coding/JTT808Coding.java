...
import kuyou.common.bytes.BCD8421Operater;
...
public class JTT808Coding {
     ...
     * @return
     */
    public static byte[] generate808(int msgId, RemoteControlDeviceConfig config, byte[] msgBody) {
    //public static byte[] generate808(int msgId, String phone, byte[] msgBody) {
        return generate808(msgId, config, msgBody, 0, 0, 0);
    }
     ...
     * @param pkgNo      当前序号
     */
    public static byte[] generate808(int msgId, RemoteControlDeviceConfig config, byte[] msgBody, int subpackage, long totalPkg, long pkgNo) {
        ...
        if (subpackage == 1) {
            ...
            msgHeader = ByteUtils.byteMergerAll(msgIdb, msgBodyAttributes, terminalPhone, flowNum, totalPkgB, pkgNob);
        } else {
            msgHeader = ByteUtils.byteMergerAll(msgIdb, msgBodyAttributes, terminalPhone, flowNum);
        }
        //=========================数据合并（消息头，消息体）=====================//
        byte[] bytes = ByteUtils.byteMergerAll(msgHeader, msgBody);
        ...
        byte[] AllData = ByteUtils.byteMergerAll(bytes, checkCode);
        ...
        //=========================最终传输给服务器的数据==================================//
        return ByteUtils.byteMergerAll(flag, replaceByte, flag);
    }



    public static Generate808andSeqBean generate808orSeqNo(int msgId, RemoteControlDeviceConfig config, byte[] msgBody) {
        return generate808orSeqNo(msgId, config, msgBody,0,0,0);
    }
    /**
     * 包装808数据,分包消息
     *
     ...
     * @return 包装好的数据和消息序号
     */
    public static Generate808andSeqBean generate808orSeqNo(int msgId, RemoteControlDeviceConfig config, byte[] msgBody, int subpackage, long totalPkg, long pkgNo) {
        ...
        //=========================最终传输给服务器的数据==================================//
        byte[] generate808 = ByteUtils.byteMergerAll(flag, replaceByte, flag);
    ...
    public static byte[] check808DataThrows(byte[] bytes) throws SocketManagerException {
        ...
        String sumCode = HexUtil.getBCC(data);
        if (!checkCode.equals(sumCode)) {
            Log.e("kuyou.sdk.jt808.base.jt808coding > JTT808Coding", new StringBuilder()
                    .append("check808DataThrows > process fail : ")
                    .append("校验码失败： ").append(HexUtil.byte2HexStr(bytes))
                    .toString());
    ...
    public static JTT808Bean resolve808(byte[] bytes) {
        ...
        jtt808.setReplyFlowNumber(msgFlowNum); //应答流水号 //临时修改
        //注册或重置鉴权码数据包
    ...
    public static Header808Bean resolve808ToHeader(byte[] bytes) {
        ...
        header808.setMsgID(ByteUtils.bytes2Int(msgId));
        header808.setMobile(HexUtil.byte2HexStrNoSpace(phone));
        header808.setSeqNO(ByteUtils.bytes2Int(msgFlowNum));
        ...
        String bodyAtteBit = ByteUtils.byteToBit(msgBodyAttributes[0]); //取到前8位
    ...
    private static byte[] msgBodyAttributes(int msgLength, int subpackage) {
        ...
        String encryption = RemoteControlDeviceConfig.Header.ENCRYPT;
        //[13]分包
        String subpackageB = String.valueOf(subpackage);
        //[14,15]保留位
        String reserve = RemoteControlDeviceConfig.Header.RESERVE;
