...
public class JT808Directive {
    ...
    public static byte[] locationReport(byte alarmType, String terminalId, long lat, long lng, int alarmId, double speed) {
        ...
        //2进制转int 在装4个字节的byte
        byte[] state = ByteUtils.int2Bytes(Integer.parseInt(radix2State, 2));
        //DWORD经纬度
    ...
    public static byte[] advancedDriverAssistance(byte alarmType, String terminalId, byte[] lat, byte[] lng,
        ...
        //报警ID转4个字节的byte
        byte[] id = ByteUtils.int2Bytes(alarmId);
        //速度转byte
     ...
     * @return
     */
    public static byte[] reportLocation(Jt808Config config, double lat, double lng, double altitude, float speed, float bearing, float accuracy, String time) {
        byte[] alarm = {0, 0, 0, 0};
//        byte[] state = {0, 0, 0, 0};
    ...
    public static byte[] reportLocation(Jt808Config config, Jt808MapLocation jtData) {
     ...
     * 发送心跳
     */
    public static byte[] heartPkg(Jt808Config config) {
