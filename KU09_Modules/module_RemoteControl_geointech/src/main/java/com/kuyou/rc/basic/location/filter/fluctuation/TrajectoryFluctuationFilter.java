package com.kuyou.rc.basic.location.filter.fluctuation;

import android.util.Log;

import com.kuyou.rc.basic.location.filter.TrackPoint;
import com.kuyou.rc.basic.location.filter.TrajectoryFilter;

import java.util.ArrayList;
import java.util.List;

import kuyou.common.utils.GPSUtils;

/**
 * action : 波动滤波器[速度/角速度/海拔]
 * <p>
 * author: wuguoxian <br/>
 * date: 20200421 <br/>
 * <p>
...
public class TrajectoryFluctuationFilter extends TrajectoryFilter {
    ...
    * */
    private TrackPoint traceFilterBySpeed(TrackPoint point) {
        ...
        //避免位置重复,轨迹集合超过10分钟后失效
        if (mTrackPointList.size() >= 1) {
            if (point.getTime() - mTrackPointList.get(mTrackPointList.size() - 1).getTime() > 1000 * 60 * 10) {
