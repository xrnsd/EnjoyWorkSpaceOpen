...
public class TouchGestureManager extends OrientationEventListener {
    protected static final String TAG = "kuyou.common.gestures > TouchGestureManager";
    ...
    public void onOrientationChanged(int orientation) {
        if (mTouchGestures == null) {
            Log.e(TAG, "onOrientationChanged>  mTouchGestures is null");
            return;
        }
        final int count = 100;
        if (mRects == null) {
    ...
    private int getGestureIndexByDownPoint(final float down_x, final float down_y) {
        if (mRects == null) {
            Log.e(TAG, "getGestureIndexByDownPoint>  mRects is null");
            return -1;
        }
        int x = (int) down_x, y = (int) down_y;
        for (int index = 0, length = mRects.size(); index < length; index++) {
            if (mRects.get(index).contains(x, y)) {
                Log.d(TAG, "getGestureIndexByDownPoint>  rect =" + mRects.get(index) + "   index=" + index);
                return index;
            }
        }
        return -1;
